...
export default function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/home" className="text-[#D4AF37] hover:underline">
            Voltar para início
          </Link>
        </div>
      </div>
  ...
  const getReputacaoColor = (nivel: NivelReputacao) => {
    switch (nivel) {
      ...
      case 'confiavel': return 'from-blue-500 to-blue-600';
      case 'atencao': return 'from-yellow-500 to-yellow-600';
      case 'perigo': return 'from-red-500 to-red-600';
    }
  };
  ...
  const calcularMediaCategoria = (categoria: string) => {
    if (perfil.avaliacoes.length === 0) return 0;
    const soma = perfil.avaliacoes.reduce((acc, av) => {
      const valor = (av as any)[categoria];
      return acc + (typeof valor === 'number' ? valor : 0);
    }, 0);
    return Number((soma / perfil.avaliacoes.length).toFixed(1));
  };
    ...
    <div className="min-h-screen bg-black text-white pb-20">
      ...
      <header className="bg-gradient-to-b from-black to-black/95 border-b border-[#D4AF37]/20 sticky top-0 z-40">
        <div className="max-w-md mx-auto px-4 py-4">
          <Link href="/home" className="flex items-center gap-2 text-[#D4AF37] hover:text-[#C0C0C0]">
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <div className="bg-gradient-to-br from-white/10 to-white/5 border border-[#D4AF37]/30 rounded-2xl p-6 mb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              {perfil.idade && <p className="text-gray-400 text-sm">{perfil.idade} anos</p>}
            </div>

            <div className={`px-4 py-2 rounded-full text-sm font-semibold bg-gradient-to-r ${getReputacaoColor(perfil.nivelReputacao)} text-white`}>
        ...
        <div className="bg-gradient-to-br from-[#D4AF37]/20 to-[#C0C0C0]/10 border border-[#D4AF37]/30 rounded-2xl p-6 mb-6">
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="text-5xl font-bold text-white">{perfil.notaGeral.toFixed(1)}</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="text-center">
              <p className="text-3xl font-bold text-[#D4AF37]">{perfil.porcentagemConfiabilidade}%</p>
        ...
        {perfil.alertas.length > 0 && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-6 mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold text-red-400">Alertas de Segurança</h2>
            </div>

            <div className="space-y-3">
              {perfil.alertas.map((alerta) => (
                <div key={alerta.id} className="bg-black/30 border border-red-500/20 rounded-xl p-4">
                  <div className="flex items-start justify-between mb-2">
                    <p className="text-sm font-semibold text-red-400 capitalize">{alerta.tipo}</p>
        ...
        {perfil.avaliacoes.length > 0 && (
          <div className="bg-white/5 border border-[#D4AF37]/20 rounded-2xl p-6 mb-6">
            <h2 className="text-lg font-semibold text-[#D4AF37] mb-4">Avaliações por Categoria</h2>

            <div className="space-y-4">
              {categorias.map((categoria) => {
                const media = calcularMediaCategoria(categoria.key);
                ...
                return (
                  <div key={categoria.key}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-semibold text-white">{media}/5.0</span>
                    </div>

                    <div className="w-full bg-white/10 rounded-full h-2">
                      <div
                        className="bg-gradient-to-r from-[#D4AF37] to-[#C0C0C0] h-2 rounded-full"
        ...
        {/* Avaliações (Relatos) */}
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-[#D4AF37] mb-4">Relatos de Usuárias</h2>
          ...
          {perfil.avaliacoes.length === 0 ? (
            ...
            <div className="space-y-4">
              {perfil.avaliacoes.map((avaliacao) => {
                ...
                return (
                  <div key={avaliacao.id} className="bg-white/5 border border-[#D4AF37]/20 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <Star className="w-4 h-4 text-[#D4AF37] fill-[#D4AF37]" />
                        <span className="text-sm font-semibold text-white">{notaFinal}</span>
                      </div>
                    ...
                    {avaliacao.redFlags.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {avaliacao.redFlags.map((flag, index) => (
                          <span key={index} className="text-xs px-2 py-1 bg-red-500/20 text-red-400 rounded-full border border-red-500/30">
                            {flag}
                          </span>
                        ))}
                      </div>
        ...
        <Link
          href="/avaliar"
          className="block w-full bg-gradient-to-r from-[#D4AF37] to-[#C0C0C0] text-black font-semibold py-4 rounded-xl text-center hover:opacity-90"
        >
          Avaliar Este Perfil
        </Link>
      </div>
